...
export function ImportForm() {
  const [state, formAction, pending] = useActionState(importProductsAction, {});
    ...
    <form action={formAction} className="grid gap-4 rounded-lg border border-white/10 bg-white/[0.03] p-4">
      ...
      {state.message ? (
        <p
          className={
            state.ok
              ? "whitespace-pre-line rounded-md border border-teal-400/20 bg-teal-500/10 p-3 text-sm leading-7 text-teal-100"
              : "whitespace-pre-line rounded-md border border-red-400/20 bg-red-500/10 p-3 text-sm leading-7 text-red-100"
          }
        >
          {state.message}
        </p>
